function Isosurfaces( volume, isovalue )
{
  ...
  for ( var z = 0; z < volume.resolution.z - 1; z++ )
  {
      for ( var y = 0; y < volume.resolution.y - 1; y++ )
      {
          for ( var x = 0; x < volume.resolution.x - 1; x++ )
          {
              ...
              for ( var j = 0; lut.edgeID[index][j] != -1; j += 3 )
              {
                  ...
                  var v01 = interpolated_vertex( v0, v1, volume.values[cell_index] );
                  var v23 = interpolated_vertex( v2, v3, volume.values[cell_index] );
                  var v45 = interpolated_vertex( v4, v5, volume.values[cell_index] );
                  ...
                  face.color.set(new THREE.Color().setHex( cmap[volume.values[cell_index]][1] ));
    ...
    {
        return new THREE.Vector3().addVectors( v0.multiplyScalar(s), v1.multiplyScalar(255-s) ).divideScalar( 255 );
    }
}
